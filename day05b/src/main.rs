...
fn exec_move(s: &str, mut stacks: Vec<Vec<char>>) -> Vec<Vec<char>> {
    let mut swap_buffer = vec![0 as char; 64];
    ...
    let swap =  &mut swap_buffer[..n];

    let len = stacks[src-1].len();
    swap.copy_from_slice(&stacks[src-1][len-n..len]);
    stacks[src-1].truncate(len-n);
    stacks[dest-1].extend(swap.iter());
