...
type Cave = HashMap<usize, HashSet<usize>>;

trait CaveTrait {
    fn insert_point(&mut self, p: (usize, usize));
    fn is_blocked(&self, p: (usize, usize)) -> bool;
    fn insert_sand(&mut self, bounds: &Bounds) -> bool;
}

impl CaveTrait for Cave {

    fn insert_point(&mut self, (x, y): (usize, usize)) {
        let set = match self.get_mut(&x) {
            Some(set) => set,
            None => {
                self.insert(x, HashSet::new());
                self.get_mut(&x).unwrap()
            },
        };
        set.insert(y);
    }
...
fn main() {
    ...
    let mut bounds = Bounds::from(&[(500, 0)]);
    ...
    let mut counter = 0;
    while cave.insert_sand(&bounds) {
        counter+=1;
    }

    println!("{}", bounds.to_string());
    println!("{}", counter);
}
